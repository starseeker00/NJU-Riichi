	...
	async fetch(request, env) {
		...
		if (pathname.startsWith('/api/')) {
			...
			switch (pathname) {
				...
				case '/api/getContestTeamList': {
					const contestId = Number(searchParams.get('contestId') || 1);
					data = await getContestTeamList(db, contestId);
					break;
				}
			}
...
WHERE GameRecords.uuid = RecordDetail.uuid AND GameRecords.schedule>=0 AND GameRecords.uuid = ? \
...
async function getContestPlayerList(db, contestId) {
	const records = await db.prepare("\
		SELECT \
		  user_id, team_id, team_name, MIN(RecordDetail.username) as username, \
		 ...
		 FROM GameRecords, RecordDetail \
		 LEFT JOIN \
		  TeamMembers ON RecordDetail.username = TeamMembers.username \
		 WHERE GameRecords.uuid = RecordDetail.uuid AND GameRecords.contest_id = ? AND GameRecords.schedule=1 AND RecordDetail.user_id > 0 \
		 GROUP BY RecordDetail.user_id \
	...
	//    WHERE r1.uuid = r2.uuid AND name <> opp_name AND opp_uid > 0 \
...
async function getContestTeamList(db, contestId, schedule = 1) {
	const records = await db.prepare("\
SELECT \
    TeamMembers.team_id, \
    TeamMembers.team_name, \
    SUM( IFNULL(RecordDetail.accuracy, 0) ) AS ttl_accuracy, \
    COUNT(RecordDetail.uuid) AS ttl_match, \
    COUNT(CASE WHEN RecordDetail.rank = 1 THEN 1 END) AS ttl_one, \
    COUNT(CASE WHEN RecordDetail.rank = 2 THEN 1 END) AS ttl_two, \
    COUNT(CASE WHEN RecordDetail.rank = 3 THEN 1 END) AS ttl_three, \
    COUNT(CASE WHEN RecordDetail.rank = 4 THEN 1 END) AS ttl_four \
FROM \
	TeamMembers \
LEFT JOIN \
	GameRecords ON GameRecords.contest_id = TeamMembers.contest_id \
LEFT JOIN \
    RecordDetail ON RecordDetail.username = TeamMembers.username \
