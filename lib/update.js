...
const fetchContestRecordIds = async (conn, contest_id, lastime = 0) => {
    ...
    const sql = `INSERT OR REPLACE INTO Contests (contest_short_id, contest_id, name, nickname, start_time, finish_time, game_mode, rule, description ) VALUES (${contest_id}, ${unique_id}, '${contest_name}', '${contest_name}', ${start_time}, ${finish_time}, ${game_mode}, ${rank_rule}, '${private_notice}');`
...
function decodeGameRecord(decoder, game, schedule, remark, divide_equally = false) {
    if (game.error) {
    ...
    // 同分平分顺位马
    if (divide_equally) {
        const groupedByScore = result.reduce((acc, player) => {
            if (!acc[player.score]) {
                acc[player.score] = [];
            }
        ...
        for (const players of Object.values(groupedByScore)) {
            if (players.length > 1) {
                const averageAccuracy = players.reduce((sum, player) => sum + player.accuracy, 0) / players.length;
                players.forEach(player => {
                    player.accuracy = averageAccuracy;
                });
            }
        }
    }
            ...
            case ".lq.RecordHule": {
                ...
                for (const info of res.data.hules) {
                    ...
                    const fanNames = ['', '门清', '立直', '枪杠', '岭上开花',
                        ...
                        '一发', '宝牌', '赤宝牌', '里宝牌', '拔北宝牌',
                        '天和', '地和', '大三元', '四暗刻', '字一色',
                        '绿一色', '清老头', '国士无双', '小四喜', '四杠子',
                        '九莲宝灯', '大四喜', '纯正九莲', '四暗刻单骑', '国士十三面',
                    ]
                    let critical = 0
                    if (info.title_id >= 3) {
...
const schedule_280319 = (uuid) => {
    if (uuid.substring(0, 6) < '250201') return { schedule: -1, remark: '' }
    return { schedule: 1, remark: '初赛' }
}
...
const schedule_292435 = (uuid) => {
    if (uuid.substring(0, 6) <= '250810')
        return { schedule: 1, remark: '海选赛' }
}
...
async function retrieveGameRecord(game_uuid) {
    ...
    // const { basic, result } = decodeGameRecord(conn._codec._wrapper, game_record)
    const { basic, result } = decodeGameRecord(conn._codec._wrapper, game_record, 1, '常规赛', 1)
...
// retrieveGameRecord('250203-f2b6f428-cb85-4b29-aa45-3fbf787fecb7')
