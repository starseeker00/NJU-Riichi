    ...
    return new Promise((resolve) => {
        ...
        const intervalId = setInterval(async () => {
            if (index >= uuids.length) {
                ...
                fs.writeFileSync(filePath, String(first_time || 0));
...
const watch_list = [130289, 942116]
...
async function update(watch_list, online = true) {
    ...
    const process = watch_list.map((contest_id) => {
        if (online) {
            return fetchAndAnalyzeGameRecord(conn, contest_id, schedule_map[contest_id])
        } else {
            return analyzeGameRecordFromFiles(contest_id, schedule_map[contest_id])
        }
    })
