...
const analyzeGameRecord = async (conn, game_uuid) => {
    const game_record = await conn.rpcCall(".lq.Lobby.fetchGameRecord", { game_uuid })
    dump_json_record(game_record, game_uuid)
        ...
        switch (res.name) {
            ...
            case ".lq.RecordHule": {
                for (const info of res.data.hules) {
                    ...
                    if (info.liqi) {
                        if (info.fans.find((fan) => fan.id === 33)?.val) {
...
function load_json_record(uuid) {
    return JSON.parse(fs.readFileSync(`lib/data/GameRecord/${uuid}.json`, 'utf8'));
}

function dump_json_record(record, uuid) {
    fs.writeFileSync(`lib/data/GameRecord/${uuid}.json`, JSON.stringify(record, null, 2));
}
...
async function main() {
    ...
    const { first_time, uuids } = await fetchContestRecordIds(conn, contest_id)
    // console.log(uuids)

    let sqls = []
    for (const game_uuid of uuids) {
        const { basic, result } = await analyzeGameRecord(conn, game_uuid)
        sqls.push(toGameRecordSql(basic))
        sqls.push(...toRecordDetailSql(result))
    }
    fs.writeFileSync(`lib/data/sql/update-${contest_id}-${first_time}.sql`, sqls.join('\n'))
    conn.close()
}

function analyzeGameRecordFromFiles() {
    const uuids = fs.readdirSync('lib/data/GameRecord')

    let sqls = []
    for (const uuid of uuids) {
        const game = load_json_record(uuid.replace('.json', ''))
        const { basic, result } = decodeGameRecord(decoder, game)
        sqls.push(toGameRecordSql(basic))
        sqls.push(...toRecordDetailSql(result))
    }
    fs.writeFileSync(`lib/data/sql/update.sql`, sqls.join('\n'))
}

main()

// const { basic, result } = decodeGameRecord(decoder, load_json_record('240728-078ed684-a463-4dfd-9d64-80be54d88a7f'))
// analyzeGameRecordFromFiles()
