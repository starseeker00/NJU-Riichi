// Referrer: https://github.com/SAPikachu/amae-koromo-scripts/
...
const { URL_BASE, ACCESS_TOKEN, PREFERRED_SERVER, OAUTH_TYPE } = require("./env.local");
...
class MajsoulProtoCodec {
  ...
  decodeMessage(buf, test = '') {
    ...
    if (type === REQUEST) {
      ...
      if (test) {
        methodName = test;
        const methodObj = this.lookupMethod(test);
...
class MajsoulConnection {
  ...
  async rpcCall(methodName, payload) {
    ...
    // console.log("Sending", methodName, payload);
